...
//   Skizo quick fix
//
// We don't import "mscvrt" or anything like that as all the symbols we rely
// on are imported by Skizo manually. Additionally, we get rid of
// "libtcc1.a" and the startup time reduces from 200 ms to just under 1 ms.
// However, C still requires certain default functions, such as "memset" or
// "memcpy"
// TODO check for other platforms x32-only currently :(
// ***************************************************************************

static void tcc_add_skizo_symbols(TCCState* state)
{
    tcc_add_symbol(state, "memset", (void*)&memset);
    tcc_add_symbol(state, "memcpy", (void*)&memcpy);

    // NOTE the definitions are taken from libtcc1.c (which is an unchanged TCC)
    ...
    tcc_add_symbol(state, "__tcc_fpu_control", &__tcc_fpu_control);
    tcc_add_symbol(state, "__tcc_int_fpu_control", &__tcc_int_fpu_control);
}
